...
import MkEmoji from '@/components/MkEmoji.vue';
...
export default defineComponent({
  ...
  render() {
    ...
    const genEl = (ast: mfm.MfmNode[]) =>
      ast
        .map((token, index): VNodeArrayChildren => {
          switch (token.type) {
            case 'text': {
            ...
            case 'fn': {
              ...
              switch (token.props.name) {
                ...
                case 'rotate': {
                  const rotate = token.props.args.x
                    ...
                    : token.props.args.y
                      ...
                      : 'rotate';
                  const degrees = Number.parseFloat((token.props.args.deg ?? '90').toString());
                  style = `transform: ${rotate}(${degrees}deg); transform-origin: center center;`;
                  break;
                }
                case 'position': {
                  const x = Number.parseFloat((token.props.args.x ?? '0').toString());
                  const y = Number.parseFloat((token.props.args.y ?? '0').toString());
                ...
                case 'scale': {
                  const x = Math.min(Number.parseFloat((token.props.args.x ?? '1').toString()), 5);
                  const y = Math.min(Number.parseFloat((token.props.args.y ?? '1').toString()), 5);
              ...
              if (style == null) {
                return [
                  h('span', {}, ['$[', token.props.name, ' ', ...genEl(token.children), ']']),
                ];
              } else {
                return [
                  h(
            ...
            case 'search': {
              ...
              if (token.props.content.slice(-6).toLowerCase() === 'search') {
                ...
                if (isMfmText(txtNode) && txtNode.props.text.endsWith(sentinel)) {
                ...
                if (
                  index === 0 ||
                  ['blockCode', 'center', 'mathBlock', 'quote', 'search'].includes(
                    ast[index - 1].type,
                  )
                ) {
                  prefix = '';
            ...
            default: {
              console.error('unrecognized ast type:', (token as { type: never }).type);
              return [];
            }
          }
        })
